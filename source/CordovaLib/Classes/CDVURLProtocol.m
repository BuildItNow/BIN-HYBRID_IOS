...
static CDVScriptManager *scriptManagerForRequest(NSURLRequest* request)
{
    ...
    return (__bridge CDVScriptManager*)(void*)viewControllerAddress;
}
...
// a ViewController.
+ (void)registerViewController:(CDVScriptManager*)viewController
{
...
+ (void)unregisterViewController:(CDVScriptManager*)viewController
{
    @synchronized(gRegisteredControllers) {
...
+ (BOOL)canInitWithRequest:(NSURLRequest*)theRequest
{
    ...
    CDVScriptManager* scriptManager = scriptManagerForRequest(theRequest);

    if ([[theUrl absoluteString] hasPrefix:kCDVAssetsLibraryPrefix]) {
        return YES;
    } else if (scriptManager != nil) {
        if ([[theUrl path] isEqualToString:@"/!gap_exec"]) {
            ...
            if (hasCmds) {
                ...
                [scriptManager.commandQueue performSelectorOnMainThread:sel withObject:queuedCommandsJSON waitUntilDone:NO];
            } else {
                SEL sel = @selector(maybeFetchCommandsFromJs:);
                [scriptManager.commandQueue performSelectorOnMainThread:sel withObject:[NSNumber numberWithInteger:[requestId integerValue]] waitUntilDone:NO];
            }
