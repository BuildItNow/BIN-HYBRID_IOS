...
@implementation CDVScriptManager
...
- (void) initScriptMachine
{
    ...
    if (hideKeyboardFormAccessoryBar) {
        __weak CDVScriptManager* weakSelf = self;
...
- (void) loadFromURL:(NSString*)url
{
    ...
    if(!url)
    {
        ...
        {
            url = [NSString stringWithFormat:@"%@/%@", self.wwwFolderName, self.startPage];
        }
    }
    ...
    else
    {
        ...
        {
            NSString* error = [NSString stringWithFormat:@"ERROR: Start Page at '%@/%@' was not found.", self.wwwFolderName, self.startPage];
            [self loadFromHTML:[NSString stringWithFormat:@"<html><body> %@ </body></html>", error]];
...
- (void)createGapView
{
    if (!self.webView) {
        ...
        self.webView = [self newCordovaViewWithFrame:[UIScreen mainScreen].bounds];
...
- (void) onViewControllerAttach:(UIViewController*) viewController
{
    _viewController = viewController;
}

- (void) onViewControllerDettach:(UIViewController*) viewController
{
...
{
    if ([plugin respondsToSelector:@selector(setScriptManager:)]) {
        [plugin setScriptManager:self];
    }
